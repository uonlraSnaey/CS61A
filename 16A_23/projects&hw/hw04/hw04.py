...
def balanced(m):
    ...
    if is_planet(m):
        return True
    len_left, len_right = end(left(m)), end(right(m))
    if balanced(len_left) and balanced(len_right):
        return True

    return total_weight(len_left) * length(left(m)) == total_weight(len_right) * length(right(m))
...
def totals_tree(m):
    ...
    if is_mobile(m):
        en_left, en_right = end(left(m)), end(right(m))
        # return tree(total_weight(m), [totals_tree(end(left(m))), totals_tree(end(right(m)))])
        return tree(total_weight(m), [totals_tree(en_left), totals_tree(en_right)])
