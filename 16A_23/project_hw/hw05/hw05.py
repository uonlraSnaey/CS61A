...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, code):
        nonlocal balance, password, lock
        if lock:
        ...
        if code == password:
            ...
            incorrect_attempts.append(code)
...
def repeated(t, k):
    ...
    "*** YOUR CODE HERE ***"
    """
    找出在可迭代的 t 中连续出现 k 次的元素，
    通过 next(t), 可对 t 中的元素进行迭代，每次向前迭代一位，这样就可以实现相邻项的比对
    """
    last = next(t)
    occur_times = 1
    for v in t:
    ...
    if next_a is not None or next_b is not None:
        if next_a is None:
            ...
            if next_a < next_b:
                ...
                yield next_a
                next_a, next_b = next(next_a, None), next(next_b, None)
    """
