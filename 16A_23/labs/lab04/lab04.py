...
def closer_city(lat, lon, city_a, city_b):
    ...
    "*** YOUR CODE HERE ***"
    '''
    ax = abs(get_lon(city_a) - lon)
    ay = abs(get_lat(city_a) - lat)
    distance_a = sqrt(ax**2 + ay**2)

    bx = abs(get_lon(city_b) - lon)
    by = abs(get_lat(city_b) - lat)
    distance_b = sqrt(by**2 + bx**2)
